...
enum class ECombatState : uint8
{
	...
	ECS_Setting UMETA(DisplayName = "Setting"),
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FGuideOverlapDelegate, int32, GuideTextIndx, int32, text2);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FUpgradeShopDelegate, bool, Open);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FSettingUIDelegate, bool, Open);
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FPlayerDie);
...
public:
	...
	// Setting
	void Setting(const float& MouseX, const float& MouseY, const float& MouseXAim, const float MouseYAim,
		const float& GamepadX, const float& GamepadY, const float& GamepadXAim, const float& GamepadYAim);
...
private:
	...
	void LeftWeaponPressed();
	void RightWeaponPressed();
...
private:
	...
	*/
	...
	UPROPERTY(EditAnywhere, Category = "Input | InputAction")
	UInputAction* LeftWeaponAction;

	UPROPERTY(EditAnywhere, Category = "Input | InputAction")
	UInputAction* RightWeaponAction;
	...
	*/
	...
	int32 Starting9mmAmmo{ 900 };

	// 시작시 기본 지금 5.56mm 총알
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Combat | Weapon", meta = (AllowPrivateAccess = "true"))
	int32 StartingARAmmo{ 900};
	...
	*/
	...
	// 피격 모션 Section Name
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Combat | Montage", meta = (AllowPrivateAccess = "true"))
	TArray<FName> HitReactSectionName;
	...
	*/
	...
	// Player 피격시 사운드
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Combat | Sound", meta = (AllowPrivateAccess = "true"))
	...
	*/
	...
	/**
		Shop
	*/
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Shop | Upgrade", meta = (AllowPrivateAccess = "true"))
	bool bInShop{ false };

	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Shop | Upgrade", meta = (AllowPrivateAccess = "true"))
	bool bOpenShop{ false };

	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Shop | MonsterWave", meta = (AllowPrivateAccess = "true"))
	...
	*/
	...
	FGuideOverlapDelegate GuideOverlapDelegate;

	// Upgrade Shop Open/Close
	UPROPERTY(BlueprintAssignable, Category = "Delegate", meta = (AllowPrivateAccess = "true"))
	FUpgradeShopDelegate UpgradeShopOverlapDelegate;

	// Setting UI
	UPROPERTY(BlueprintAssignable, Category = "Delegate", meta = (AllowPrivateAccess = "true"))
	FSettingUIDelegate SettingUIDelegate;
	...
	void SetMouseX(float Sensitivity);

	UFUNCTION(Exec, Category = "Command")
	void SetMouseY(float Sensitivity);

	// Tutorial
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Tutorial | Spawner", meta = (AllowPrivateAccess = "true"))
	bool bInSpawnArea{ false };

	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Tutorial | Spawner", meta = (AllowPrivateAccess = "true"))
	ASpawner* Spawner;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Tutorial | GuideText", meta = (AllowPrivateAccess = "true"))
	TArray<FText> Title;
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Tutorial | GuideText", meta = (AllowPrivateAccess = "true"))
	TArray<FText> Gamepad;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Tutorial | GuideText", meta = (AllowPrivateAccess = "true"))
	TArray<FText> Explain;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Tutorial | GuideText", meta = (AllowPrivateAccess = "true"))
	int32 GuideTextIdx{ 0 };
...
public:
	...
	FORCEINLINE float GetStunChane() const { return StunChance; }
	FORCEINLINE void AddCoin(int32 CoinAmount) { Coin += CoinAmount; }
	FORCEINLINE void SetCoin(int32 CoinAmount) { Coin = CoinAmount; }
	FORCEINLINE int32 GetCoin() const { return Coin; }
	FORCEINLINE void SetInShop(bool IsShop) { bInShop = IsShop; }
	FORCEINLINE float GetDefensive() const { return Defensive; }
	FORCEINLINE void AddDefensive(float DefensiveAmount) { Defensive += DefensiveAmount; }
	FORCEINLINE float GetHealth() const { return Health; }
	FORCEINLINE void SetHealth(float Amount) { Health = Amount; }
	FORCEINLINE void AddHealth(float Amount) { Health += Amount; }
	FORCEINLINE float GetMaxHealth() const { return MaxHealth; }
	FORCEINLINE void AddMaxHealth(float MaxHealthAmount) { MaxHealth += MaxHealthAmount; }
	FORCEINLINE void AddUpgradeDamage(float DamageAmount) { UpgradeDamage += DamageAmount; }

	FORCEINLINE void SetInSpawnArea(bool InArea) { bInSpawnArea = InArea; }
	FORCEINLINE void SetSpanwer(ASpawner* _ASpawner) { Spawner = _ASpawner; }
	FORCEINLINE TArray<FText> GetTileArray() const { return Title; }
	FORCEINLINE void SetGuideTextIdx(int TextIdx) { GuideTextIdx = TextIdx; }
	FORCEINLINE FGuideOverlapDelegate GetGuideOverlapDelegate() {  return GuideOverlapDelegate; }
	FORCEINLINE FSettingUIDelegate GetSettingUIDelegate() const { return SettingUIDelegate; }
	FORCEINLINE void SetMonsterWave(AMonsterWave* Wave) { MonsterWave = Wave; }
	FORCEINLINE AMonsterWave* GetMonsterWave() const { return MonsterWave; }
	FORCEINLINE bool GetbOpenShop() const { return bOpenShop; }
};
