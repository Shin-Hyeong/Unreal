...
void AShooterCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	if (UEnhancedInputComponent* EnhancedInputComponent = Cast<UEnhancedInputComponent>(PlayerInputComponent))
	{
		...
		EnhancedInputComponent->BindAction(LeftWeaponAction, ETriggerEvent::Started, this, &AShooterCharacter::LeftWeaponPressed);
		EnhancedInputComponent->BindAction(RightWeaponAction, ETriggerEvent::Started, this, &AShooterCharacter::RightWeaponPressed);
		EnhancedInputComponent->BindAction(SettingAction, ETriggerEvent::Started, this, &AShooterCharacter::SettingPressed);
...
void AShooterCharacter::Setting(const float& MouseX, const float& MouseY, const float& MouseXAim, const float MouseYAim,
	const float& GamepadX, const float& GamepadY, const float& GamepadXAim, const float& GamepadYAim)
{
	MouseHipTurnRate = MouseX;
	MouseHipLookUpRate = MouseY;
	MouseAimingTurnRate = MouseXAim;
	MouseAimingLookUpRate = MouseYAim;

	GamepadHipTurnRate = GamepadX;
	GamepadHipLookUpRate = GamepadY;
	GamepadAimingTurnRate = GamepadXAim;
	GamepadAimingLookUpRate = GamepadYAim;
}
...
{
	if (CombatState == ECombatState::ECS_Setting || CombatState == ECombatState::ECS_Dead) return;
...
{
	if (Controller != nullptr && (CombatState != ECombatState::ECS_Setting || CombatState != ECombatState::ECS_Dead))
	{
		ACharacter::Jump();
		bCrouching = false;
		GetCharacterMovement()->MaxWalkSpeed = BaseMovementSpeed;
...
{
	if (CombatState == ECombatState::ECS_Dead || CombatState == ECombatState::ECS_Setting) return;
...
void AShooterCharacter::AimingButtonPressed()
{
	...
	if (CombatState != ECombatState::ECS_Reloading && 
		...
		CombatState != ECombatState::ECS_Stunned &&
		CombatState != ECombatState::ECS_Setting &&
		CombatState != ECombatState::ECS_Dead)
	{
		Aim();
	}
...
{
	// Open Shop
	if (bInShop && bOpenShop == false && PlayerController)
	{
		UpgradeShopOverlapDelegate.Broadcast(true);
		bOpenShop = true;
	...
	// Close Shop
	else if (bInShop && bOpenShop && PlayerController)
	{
		UpgradeShopOverlapDelegate.Broadcast(false);
		bOpenShop = false;
...
{
	if (GetCharacterMovement()->IsFalling() == false && CombatState != ECombatState::ECS_Setting)
	{
...
void AShooterCharacter::LeftWeaponPressed()
{
	int32 LeftWeaponIndex = EquippedWeapon->GetSlotIndex() - 1;
	if (LeftWeaponIndex <= 0)
	{
		LeftWeaponIndex = 0;
	}
	
	ExchangeInventoryItems(EquippedWeapon->GetSlotIndex(), LeftWeaponIndex);
}

void AShooterCharacter::RightWeaponPressed()
{
	int32 RightWeaponIndex = EquippedWeapon->GetSlotIndex() + 1;

	if (RightWeaponIndex >= Inventory.Num() - 1)
	{
		RightWeaponIndex = Inventory.Num() - 1;
	}

	ExchangeInventoryItems(EquippedWeapon->GetSlotIndex(), RightWeaponIndex);
}

void AShooterCharacter::SettingPressed()
{
	SettingUIDelegate.Broadcast(true);
		...
		if (bBeamEnd)
		{
			...
			if (BeamHitResult.GetActor())
			{
				...
				if (HitEnemy)
				{
					...
					{
						Damage = static_cast<int32>((EquippedWeapon->GetDamage() + UpgradeDamage) * 1.5f);

						// Damage를 기준으로 +- 20% 랜덤 데미지
						float DamageMin = Damage / 0.9f;
						float DamageMax = Damage / 1.2f;
						Damage = static_cast<int32>(FMath::FRandRange(DamageMin, DamageMax));

						// Enemy의 Defensive만큼 차감
						Damage = Damage - HitEnemy->GetDefensive();
					...
					{
						Damage = EquippedWeapon->GetDamage() + UpgradeDamage;
...
void AShooterCharacter::AutoFireReset()
{
	if (CombatState == ECombatState::ECS_Stunned ||
		CombatState == ECombatState::ECS_Setting ||
		CombatState== ECombatState::ECS_Dead) return;
	CombatState = ECombatState::ECS_Unoccupied;
